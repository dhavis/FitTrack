...
export function generateMenu(prefs: NutritionPreferences): GeneratedMenu {
  ...
  const days: MenuDay[] = DAY_LABELS.map((label, i) => {
    ...
    const lunch = meal(`${p.name} plate`, [p.name, c.name, v.name, sauce.name], p.prep_friendly && c.prep_friendly, 'Same base works for leftovers');
